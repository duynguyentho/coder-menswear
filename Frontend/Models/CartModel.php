<?php
	class CartModel{		
		...
		public function cartAdd1($id,$quantity){
		    if(isset($_SESSION['cart'][$id])){
		        //nếu đã có sp trong giỏ hàng thì số lượng cong them quantity
		        $_SESSION['cart'][$id]['number'] = $_SESSION['cart'][$id]['number'] + $quantity;
		    } else {
		        //lấy thông tin sản phẩm từ CSDL và lưu vào giỏ hàng
		        //$product = db::get_one("select * from products where id=$id");
		        //---
		...
		############################################........
		// public function modelBuy()
		// {
		// 	$conn = Connection::getInstance();
		// 	$customer_id = isset($_POST["customer_id"]) ? $_POST["customer_id"]: rand(1000,2000);
		// 	$price = $_POST["price"];
		// 	$query = $conn->query("insert into orders set customer_id=:customer_id, date=now(),price=:price");
		// 	$query->fetch()
		// }
	}	
?>